...
from conftest import SEND_MESSAGE_URL, TEST_CHAT_ID, bearer
...
async def test_malformed_payload_is_422(client, tg_ok):
    resp = await client.post("/alert", content=b"not json at all", headers=bearer())
...
async def test_wrong_auth_is_401(client, amwebhook):
    resp = await client.post("/alert", json=amwebhook, headers=bearer("wrong-token"))
    assert resp.status_code == 401
